...
def predict_next_score(player, name):
    ...
    # Create a Bayesian Ridge Regression model and fit it to the data
    model = BayesianRidge().fit(X, y)

    # Predict the next total score
    next_total = model.predict([[len(totals)]])
    next_total_rounded = [round(x) for x in next_total.tolist()]
    print(f"Next possible score for {name}: {next_total_rounded[0]}")
